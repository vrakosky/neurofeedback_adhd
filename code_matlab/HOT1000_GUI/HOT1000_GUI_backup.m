...
set(handles.uipanel_ins,'Title','Status');          % Change title Instructions to Title
set(handles.text_ins,'String','');                  % Empty the string
drawnow;

if isappdata(handles.HOT1000_GUI,'nirstreamer_job') % Remove if exist
    rmappdata(handles.HOT1000_GUI,'nirstreamer_job');
end

if exist('nirstreamer_job')                         % Remove if exist
    delete(nirstreamer_job);
    clear nirstreamer_job;
end
...
nirstreamer_job = batch("nirstreamer_launcher(com_port)");  % Run the nirstreamer.jar in background

tmp_time = datetime;                                        % Get date & time
while strcmp(nirstreamer_job.State,'running')               % Update status while nirstreamer is running
...
set(handles.text_ins,'String',tmp);
setappdata(handles.HOT1000_GUI,'nirstreamer_job',nirstreamer_job);
drawnow;
...
nirstreamer_job = getappdata(handles.HOT1000_GUI,'nirstreamer_job');
delete(nirstreamer_job);
clear nirstreamer_job;
...
if isappdata(handles.HOT1000_GUI,'nirstreamer_job')
    rmappdata(handles.HOT1000_GUI,'nirstreamer_job');
end

if exist('nirstreamer_job')
    delete(nirstreamer_job);
    clear nirstreamer_job;
end
...
all_subplot_loc = [1 1; 1 2; 1 3; 2 2; 2 3; 2 3];   % Pre-determined subplot locations (max 6 plots/devices)
...
for i = 1:hot1000_device_valid                                              % For each checked device
    ...
    if getappdata(handles.HOT1000_GUI,'record_state') == 0                  % If no recording was done
        ...
        subplot_loc = all_subplot_loc(length(checkbox_loc),:);      % Determine the subplot location
        subplot1 = subplot(subplot_loc(1),subplot_loc(2), ...       % Initiate subplot
            i,'parent',handles.uipanel_plot);
...
while get(hObject,'Value')      % If recording button is on/pressed
    ...
    for i = 1:hot1000_device_valid                      % For each device checked
        ...
        eval(sprintf('setappdata(handles.HOT1000_GUI,''data_%s'',data_%s);', ...    % Save acquired data as appdata
            device_id_list{i},device_id_list{i}));
        eval(sprintf('subplot%d = subplot(subplot_loc(1),subplot_loc(2),count_plot,''Parent'',handles.uipanel_plot);',count_plot))  % Create subplot
        cla;                                            % Clear axes
        hold on;
        ...
        title(sprintf('Device %d: %s',i,device_id_list{i}));                            % Title the subplot based on device
